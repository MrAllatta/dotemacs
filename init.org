#+STARTUP: overview
#+TITLE: Emacs Configuration
#+AUTHOR: Your Name
#+DATE: [2025-03-03]
#+OPTIONS: toc:2

* Introduction
This is a structured and modular Emacs configuration refined iteratively for:
- Efficient search & completion
- Robust syntax highlighting & LSP support
- Automated debugging, recovery, and performance optimization

All sections are tangle-ready and maintain clear tracking metadata.

* Package Management
:PROPERTIES:
:LAST_UPDATED: [2025-03-03]
:CHANGELOG: Added package initialization.
:END:

#+NAME: package-init
#+BEGIN_SRC emacs-lisp :tangle yes
(require 'package)
(setq package-archives '(("melpa" . "https://melpa.org/packages/")
                         ("org"   . "https://orgmode.org/elpa/")
                         ("gnu"   . "https://elpa.gnu.org/packages/")))
(package-initialize)
(unless package-archive-contents
  (package-refresh-contents))
#+END_SRC

* Ensure correct gcc
#+BEGIN_SRC emacs-lisp :tangle yes
(setenv "CC" "/bin/gcc")
#+END_SRC
	
* UI Customization
:PROPERTIES:
:LAST_UPDATED: [2025-03-03]
:CHANGELOG: Disabled menu bar & tool bar, enabled time display, removed startup screen.
:END:

#+NAME: ui-config
#+BEGIN_SRC emacs-lisp :tangle yes
;; Disable unnecessary UI elements
(menu-bar-mode -1)
(tool-bar-mode -1)

;; Display time in the mode-line
(display-time-mode 1)
(setq display-time-format "%a %b %d %I:%M%p")

;; Disable startup screen
(setq inhibit-startup-screen t)

;; Use 'y' or 'n' instead of 'yes' or 'no' in prompts
(fset 'yes-or-no-p 'y-or-n-p)

;; Sentences end with a single space instead of two
(setq sentence-end-double-space nil)
#+END_SRC

* Keybindings
:PROPERTIES:
:LAST_UPDATED: [2025-03-03]
:CHANGELOG: Added structured keybindings section.
:END:

** Global Keybindings
#+NAME: global-keybindings
#+BEGIN_SRC emacs-lisp :tangle yes
(global-set-key (kbd "C-x g") 'magit-status)
(global-set-key (kbd "C-c b") 'consult-buffer)
(global-set-key (kbd "M-x") 'execute-extended-command)
#+END_SRC

** Window Management
#+NAME: window-management
#+BEGIN_SRC emacs-lisp :tangle yes
(global-set-key (kbd "C-x 2") 'split-window-below)
(global-set-key (kbd "C-x 3") 'split-window-right)
(global-set-key (kbd "C-x 0") 'delete-window)
(global-set-key (kbd "C-x 1") 'delete-other-windows)
#+END_SRC

* Fix Keybinding Conflicts
:PROPERTIES:
:LAST_UPDATED: [2025-03-03]
:CHANGELOG: Resolved conflict between `org-capture` and `compile`.
:END:

#+NAME: keybinding-fix
#+BEGIN_SRC emacs-lisp :tangle yes
(global-set-key (kbd "C-c C-c") 'compile)  ;; Change compile keybinding
#+END_SRC

* Theme Configuration
:PROPERTIES:
:LAST_UPDATED: [2025-03-03]
:CHANGELOG: Fixed theme loading error by ensuring `modus-themes` is initialized before calling `modus-themes-load-theme`.
:END:

** Load Modus-Themes at Startup
#+NAME: modus-themes-config
#+BEGIN_SRC emacs-lisp :tangle yes
(use-package modus-themes
  :ensure t
  :config
  ;; Ensure the package is fully loaded before applying theme
  (require 'modus-themes)

  ;; Load preferred theme at startup
  (modus-themes-select 'modus-operandi)

  ;; Improve readability and contrast
  (setq modus-themes-bold-constructs t
        modus-themes-italic-constructs t
        modus-themes-mixed-fonts t
        modus-themes-variable-pitch-ui nil
        modus-themes-scale-headings t
        modus-themes-org-blocks 'gray-background))

;; Toggle between themes with a keybinding
(defun toggle-modus-themes ()
  "Toggle between Modus Operandi (light) and Modus Vivendi (dark)."
  (interactive)
  (if (eq (car custom-enabled-themes) 'modus-operandi)
      (modus-themes-select 'modus-vivendi)
    (modus-themes-select 'modus-operandi)))

(global-set-key (kbd "<f5>") 'toggle-modus-themes)
#+END_SRC

* Search & Completion
:PROPERTIES:
:LAST_UPDATED: [2025-03-03]
:CHANGELOG: Ensured Ivy, Consult, and Company load at startup.
:END:

** Ivy Configuration (Always Loaded at Startup)
#+NAME: ivy-config
#+BEGIN_SRC emacs-lisp :tangle yes
(use-package ivy
  :ensure t
  :init
  (ivy-mode 1)
  :config
  (setq ivy-use-virtual-buffers t
        enable-recursive-minibuffers t
        ivy-count-format "(%d/%d) "
        ivy-wrap t))
#+END_SRC

** Consult Configuration (Always Available)
#+NAME: consult-config
#+BEGIN_SRC emacs-lisp :tangle yes
(use-package consult
  :ensure t
  :bind (("C-s" . consult-line)
         ("C-x b" . consult-buffer)
         ("M-g g" . consult-goto-line)
         ("M-g M-g" . consult-goto-line)
         ("C-c h" . consult-history)))
#+END_SRC

** Company Auto-Completion (Preloaded at Startup)
#+NAME: company-config
#+BEGIN_SRC emacs-lisp :tangle yes
(use-package company
  :ensure t
  :init
  (global-company-mode)
  :config
  (setq company-minimum-prefix-length 2
        company-idle-delay 0.1
        company-tooltip-align-annotations t))
#+END_SRC
* Auto-Completion with Company
:PROPERTIES:
:LAST_UPDATED: [2025-03-03]
:CHANGELOG: Fixed missing package issue by ensuring `company` is installed and loaded properly.
:END:

#+NAME: company-config
#+BEGIN_SRC emacs-lisp :tangle yes
(use-package company
  :ensure t
  :hook (prog-mode . company-mode)
  :config
  (setq company-minimum-prefix-length 2
        company-idle-delay 0.1
        company-tooltip-align-annotations t))
#+END_SRC

* Syntax Highlighting & LSP
:PROPERTIES:
:LAST_UPDATED: [2025-03-03]
:CHANGELOG: Enabled Treesitter & LSP.
:END:

* Markdown Configuration
:PROPERTIES:
:LAST_UPDATED: [2025-03-03]
:CHANGELOG: Added markdown-mode with supporting config.
:END:

#+NAME: markdown-config
#+BEGIN_SRC emacs-lisp :tangle yes
(use-package markdown-mode
  :ensure t
  :commands (markdown-mode gfm-mode)
  :mode (("README\\.md\\'" . gfm-mode)
         ("\\.md\\'"       . markdown-mode)
         ("\\.markdown\\'" . markdown-mode))
  :init
  ;; Use Pandoc for markdown commands
  (setq markdown-command "pandoc"
        ;; Better code block highlighting
        markdown-fontify-code-blocks-natively t)
  :hook
  ;; Enable soft-wrapping in markdown buffers
  (markdown-mode . visual-line-mode))
#+END_SRC

* Tree-Sitter Configuration
:PROPERTIES:
:LAST_UPDATED: [2025-03-03]
:CHANGELOG: Added automatic Tree-Sitter grammar installation and major mode remapping.
:END:

#+NAME: treesitter-config
#+BEGIN_SRC emacs-lisp :tangle yes
;; Ensure Tree-Sitter is installed and configured
(setq treesit-language-source-alist
      '((python      "https://github.com/tree-sitter/tree-sitter-python")
        (yaml        "https://github.com/ikatyang/tree-sitter-yaml")
        (latex       "https://github.com/latex-lsp/tree-sitter-latex")
        (java        "https://github.com/tree-sitter/tree-sitter-java")
;        (typescript  ("https://github.com/tree-sitter/tree-sitter-typescript" "master" "typescript"))
        (javascript  "https://github.com/tree-sitter/tree-sitter-javascript")
        (markdown    "https://github.com/ikatyang/tree-sitter-markdown")
        (org         "https://github.com/milisims/tree-sitter-org")))

(defun ensure-treesitter-grammars-installed ()
  "Ensure all required Tree-Sitter grammars are installed."
  (message "🔍 Tree-sitter: Checking installed grammars...")
  (dolist (lang '(python yaml latex java javascript markdown org))
    (if (treesit-language-available-p lang)
        (message "✅ Tree-sitter: %s already installed." lang)
      (progn
        (message "⚠️ Tree-sitter: Installing %s..." lang)
        (treesit-install-language-grammar lang)))))

(add-hook 'after-init-hook #'ensure-treesitter-grammars-installed)

;; Use Tree-Sitter modes where available
(setq major-mode-remap-alist
      '((python-mode        . python-ts-mode)
        (yaml-mode          . yaml-ts-mode)
        (latex-mode         . latex-ts-mode)
        (java-mode          . java-ts-mode)
;        (typescript-mode    . typescript-ts-mode)
        (js-mode            . javascript-ts-mode)
        (markdown-mode      . markdown-ts-mode)
        (org-mode           . org-ts-mode)))
#+END_SRC

** LSP Integration
#+NAME: lsp-config
#+BEGIN_SRC emacs-lisp :tangle yes
(use-package lsp-mode
  :commands lsp
  :hook ((python-mode . lsp)
         (js-mode . lsp)
         (rust-mode . lsp)
         (go-mode . lsp)
         (c-mode . lsp))
  :config
  (setq lsp-keymap-prefix "C-c l"
        lsp-enable-symbol-highlighting t))
#+END_SRC

* Debugging & Recovery
:PROPERTIES:
:LAST_UPDATED: [2025-03-03]
:CHANGELOG: Implemented automated recovery & logging.
:END:

** Config Backup & Recovery
#+NAME: config-backup
#+BEGIN_SRC emacs-lisp :tangle yes
(defun backup-init-file ()
  "Backup Emacs configuration before reloading."
  (let ((backup-file "~/.emacs.d/init.bak"))
    (copy-file user-init-file backup-file t)))

(defun restore-init-file ()
  "Restore the last working Emacs configuration."
  (interactive)
  (let ((backup-file "~/.emacs.d/init.bak"))
    (if (file-exists-p backup-file)
        (copy-file backup-file user-init-file t)
      (message "No backup found!"))))

(add-hook 'before-init-hook 'backup-init-file)
#+END_SRC

** Error Logging
#+NAME: error-logging
#+BEGIN_SRC emacs-lisp :tangle yes
(setq debug-on-error t)

(defun log-emacs-errors ()
  "Log errors during startup."
  (let ((log-file "~/.emacs.d/emacs-errors.log"))
    (with-temp-file log-file
      (insert (format "Last Emacs error log: %s\n\n" (current-time-string)))
      (insert (with-output-to-string (backtrace))))))

(add-hook 'emacs-startup-hook
          (lambda () (when debug-on-error (log-emacs-errors))))
#+END_SRC

* Performance Optimization
:PROPERTIES:
:LAST_UPDATED: [2025-03-03]
:CHANGELOG: Added lazy loading & garbage collection optimization.
:END:

** Lazy Loading with `use-package`
#+NAME: lazy-loading
#+BEGIN_SRC emacs-lisp :tangle yes
(setq use-package-always-defer t)
(use-package ivy :defer 1 :config (ivy-mode 1))
(use-package company :hook (prog-mode . company-mode))
(use-package lsp-mode :hook ((python-mode . lsp) (js-mode . lsp)))
#+END_SRC

** Optimize Garbage Collection
#+NAME: gc-optimization
#+BEGIN_SRC emacs-lisp :tangle yes
(setq gc-cons-threshold (* 50 1000 1000))
(add-hook 'emacs-startup-hook
          (lambda () (setq gc-cons-threshold (* 2 1000 1000))))
#+END_SRC

* Unified Search System
:PROPERTIES:
:LAST_UPDATED: [2025-03-03]
:CHANGELOG: Added dynamic search selection.
:END:

** Unified Search Command
#+NAME: unified-search
#+BEGIN_SRC emacs-lisp :tangle yes
(defun consult-unified-search ()
  "Unified search interface for LSP symbols, project files, and ripgrep."
  (interactive)
  (let ((choice (completing-read "Search Type: " '("LSP Symbols" "Project Files" "Ripgrep"))))
    (cond
     ((string= choice "LSP Symbols") (call-interactively 'consult-lsp-symbols))
     ((string= choice "Project Files") (call-interactively 'consult-find))
     ((string= choice "Ripgrep") (call-interactively 'consult-ripgrep)))))

(global-set-key (kbd "C-c u") 'consult-unified-search)
#+END_SRC

* Org-Agenda Keybindings
:PROPERTIES:
:LAST_UPDATED: [2025-03-03]
:CHANGELOG: Added common Org-Agenda keybindings.
:END:

#+NAME: org-agenda-keybindings
#+BEGIN_SRC emacs-lisp :tangle yes
    (use-package org
      :bind (("C-c a" . org-agenda)   ;; Open Org-Agenda
             ("C-c l" . org-store-link) ;; Store link
             ("C-c c" . org-capture)   ;; Capture new note
             ("C-c b" . org-switchb)   ;; Switch Org buffer
             ("C-c d" . org-deadline)  ;; Set deadline
             ("C-c s" . org-schedule)  ;; Schedule task
             ("C-c ." . org-time-stamp) ;; Insert timestamp
             ("C-c ," . org-priority))
      :config
      (setq org-agenda-files (directory-files-recursively "~/orgfiles/" "\\.org$"))) ;; Set priority
#+END_SRC
* Org-Capture Templates
:PROPERTIES:
:LAST_UPDATED: [2025-03-03]
:CHANGELOG: Added Org-Capture templates for tasks, notes, and meetings.
:END:

#+NAME: org-capture-templates
#+BEGIN_SRC emacs-lisp :tangle yes
  (setq org-capture-templates
        '(("t" "Task" entry
           (file+headline "~/orgfiles/tasks.org" "Tasks")
           "* TODO %?\n  %U\n  %a\n")

          ("n" "Note" entry
           (file+headline "~/orgfiles/notes.org" "Notes")
           "* %?\n  %U\n  %a\n")

  	("s" "Schedule a Meeting" entry
         (file+headline "~/orgfiles/schedule.org" "Upcoming Meetings")
         "* TODO Meeting with %^{Who}\n  SCHEDULED: %^T\n  %?")))
#+END_SRC
* Org-Roam Integration (Ensure Files Appear in Search)
:PROPERTIES:
:LAST_UPDATED: [2025-03-03]
:CHANGELOG: Fixed missing Org-Roam files in search by ensuring database sync and proper metadata.
:END:

#+NAME: org-roam-config
#+BEGIN_SRC emacs-lisp :tangle yes
      (use-package org-roam
        :ensure t
        :init
        (setq org-roam-directory (expand-file-name "~/orgfiles/roam/"))
        (setq org-roam-list-files-commands '(find rg))  ;; Ensure recursive search

        (unless (file-exists-p org-roam-directory)
          (make-directory org-roam-directory t))

        ;; Auto-create missing files with `#+title:` metadata
        (dolist (file '("index.org" "projects.org" "notes.org" "meetings.org"))
          (let ((file-path (expand-file-name file org-roam-directory)))
            (unless (file-exists-p file-path)
              (with-temp-file file-path
                (insert "#+title: " (capitalize (file-name-sans-extension file)) "\n"
                        "#+OPTIONS: toc:2\n"
                        "#+STARTUP: overview\n"
                        "#+TOC: headlines 2\n\n"
                        (cond
                         ((string= file "index.org")
                          "* Welcome to Org-Roam\nYour knowledge base starts here.\n\n"
                          "* Topics\n"
                          "- [[file:projects.org][Projects]]\n"
                          "- [[file:notes.org][Notes]]\n"
                          "- [[file:meetings.org][Meetings]]\n\n"
                          "* Daily Notes\nCapture your daily thoughts using `C-c r d`.\n")

                         ((string= file "projects.org")
                          "* Projects\n\n** Active Projects\n- TODO Define active projects\n\n** Completed Projects\n- DONE Example Project\n")

                         ((string= file "notes.org")
                          "* Notes\n\n** General Notes\n- Add new notes here\n\n** References\n- Link important references\n")

                         ((string= file "meetings.org")
                          "* Meeting Notes\n\n** Upcoming Meetings\n- Capture agenda for future meetings\n\n** Past Meetings\n- Review past meetings\n")))))))

        :custom
        (org-roam-completion-everywhere t)
        :bind (("C-c r f" . org-roam-node-find)
               ("C-c r i" . org-roam-node-insert)
               ("C-c r t" . org-roam-tag-add)
               ("C-c r d" . org-roam-dailies-capture-today)
  	     ("C-c r m" . org-roam-capture))
        :config
        ;; Ensure Org-Roam database syncs at startup
        (org-roam-db-autosync-mode)
        (add-hook 'after-init-hook #'org-roam-db-sync)) ;; Ensure database is updated on startup


#+END_SRC

* Org-Roam Fix: Search in Subdirectories and Additional Directories
:PROPERTIES:
:LAST_UPDATED: [2025-03-03]
:CHANGELOG: Extended Org-Roam to search in subdirectories and multiple locations.
:END:

#+NAME: org-roam-multi-directory-fix
#+BEGIN_SRC emacs-lisp :tangle yes
(setq org-roam-extra-directories '("~/knowledge/"))

(defun my-org-roam-list-files ()
  "Return a list of all Org-Roam files in multiple directories, including subdirectories."
  (let ((org-roam-files
         (append
          (org-roam--list-files-elisp org-roam-directory)
          (org-roam--list-files-elisp (expand-file-name "~/knowledge/")))))
    (delete-dups org-roam-files)))

(advice-add 'org-roam-list-files :override #'my-org-roam-list-files)
#+END_SRC

* Org-Roam Fix: Assign Unique IDs to All Files
:PROPERTIES:
:LAST_UPDATED: [2025-03-03]
:CHANGELOG: Automatically assign Org-Roam IDs to all missing files.
:END:

#+NAME: org-roam-id-fix
#+BEGIN_SRC emacs-lisp :tangle yes
(defun ensure-org-roam-files-have-ids ()
  "Ensure every Org-Roam file has a #+title: property and a unique ID."
  (dolist (file (org-roam-list-files))
    (with-temp-buffer
      (insert-file-contents file)
      (let (needs-save)
        ;; Ensure the file has a title
        (unless (re-search-forward "^#\\+title:" nil t)
          (goto-char (point-min))
          (insert "#+title: " (file-name-base file) "\n")
          (setq needs-save t))
        ;; Ensure the file has an Org-Roam ID
        (unless (re-search-forward ":ID:" nil t)
          (goto-char (point-min))
          (insert ":PROPERTIES:\n:ID: " (org-id-new) "\n:END:\n\n")
          (setq needs-save t))
        ;; Save only if changes were made
        (when needs-save
          (write-file file))))))

;; Run this function after database sync to fix missing IDs
(add-hook 'org-roam-db-sync-hook #'ensure-org-roam-files-have-ids)
#+END_SRC

* Org-Roam Capture Templates
:PROPERTIES:
:LAST_UPDATED: [2025-03-03]
:CHANGELOG: Added structured capture templates for Org-Roam.
:END:

#+NAME: org-roam-capture-templates
#+BEGIN_SRC emacs-lisp :tangle yes
    (setq org-roam-capture-templates
          '(("d" "Default" plain
             "%?"
             :if-new (file+head "%<%Y-%m-%d>-${slug}.org"
                                "#+title: ${title}\n#+date: %U\n")
             :unnarrowed t)

            ("p" "Project" plain
             "* Goals\n%?\n* Notes\n- %U\n- Related: %a\n"
             :if-new (file+head "projects/%<%Y-%m-%d>-${slug}.org"
                                "#+title: ${title}\n#+filetags: :project:\n")
             :unnarrowed t)
	     
("c" "CLI Command Knowledge" plain
"* ${title}\n  :PROPERTIES:\n  :ID: %(org-id-new)\n  :END:\n  #+title: ${title}\n  #+filetags: :cli:commands:\n  #+created: %U\n\n** Summary\n${summary}\n\n** Command Syntax\n#+begin_src bash\n${command}\n#+end_src\n\n** Explanation\n${explanation}\n\n** Example\n${example}\n\n** Notes\n${notes}\n\n** Related Commands\n- ${related}\n"
:target (file+olp "cli-commands.org" ("CLI Commands"))
:unnarrowed t
:empty-lines 1
:prepend t
:kill-buffer t)


  	  ("m" "Meeting Notes" plain
         "* MEETING with %^{Who} on %^T\n  :PROPERTIES:\n  :ID: %(org-id-new)\n  :END:\n  #+title: Meeting - %^{Who} - %<%Y-%m-%d>\n\n** Agenda\n%?\n\n** Notes\n- "
         :target (file+olp "meetings.org" ("Meetings"))
         :unnarrowed t
         :empty-lines 1
         :prepend t
         :kill-buffer t)))
#+END_SRC

* Org-Agenda Enhancements
:PROPERTIES:
:LAST_UPDATED: [2025-03-03]
:CHANGELOG: Optimized Org-Agenda for advanced task tracking.
:END:

#+NAME: org-agenda-config
#+BEGIN_SRC emacs-lisp :tangle yes
(setq org-agenda-files '("~/orgfiles/tasks.org"
                         "~/orgfiles/projects.org"
                         "~/orgfiles/meetings.org"))

(setq org-agenda-custom-commands
      '(("d" "Daily Overview"
         ((agenda "" ((org-agenda-span 'day)))
          (todo "TODO")
          (tags "project")))

        ("w" "Weekly Plan"
         ((agenda "" ((org-agenda-span 'week)))
          (todo "NEXT")))

        ("p" "Project Overview"
         ((tags-todo "project")
          (todo "WAITING")))))

(setq org-deadline-warning-days 7) ;; Show upcoming deadlines 7 days in advance
#+END_SRC
* Org-Habit Tracking
:PROPERTIES:
:LAST_UPDATED: [2025-03-03]
:CHANGELOG: Integrated habit tracking into Org-Agenda.
:END:

#+NAME: org-habit-config
#+BEGIN_SRC emacs-lisp :tangle yes
(require 'org-habit)  ;; Ensure org-habit is loaded
(add-to-list 'org-modules 'org-habit)

(setq org-habit-graph-column 50)  ;; Align habit tracking visuals
(setq org-habit-preceding-days 7) ;; Show past 7 days of habit tracking
(setq org-habit-following-days 7) ;; Show next 7 days of habit tracking
#+END_SRC
* Force Load of Critical Features at Startup
:PROPERTIES:
:LAST_UPDATED: [2025-03-03]
:CHANGELOG: Ensured Org-mode, Org-Roam, and keybindings load immediately at startup.
:END:

#+NAME: force-load-config
#+BEGIN_SRC emacs-lisp :tangle yes
(require 'org)
(require 'org-agenda)
(require 'org-roam)
(global-set-key (kbd "C-c r f") 'org-roam-node-find) ;; Ensure Org-Roam loads
#+END_SRC
* Performance Optimization: Lazy-Load Heavy Packages
:PROPERTIES:
:LAST_UPDATED: [2025-03-03]
:CHANGELOG: Improved startup time by lazy-loading Org-Roam and Magit.
:END:

#+NAME: lazy-load-config
#+BEGIN_SRC emacs-lisp :tangle yes
(use-package org-roam
  :defer 2 ;; Load 2 seconds after startup
  :config (org-roam-db-autosync-mode))

(use-package magit
  :defer 3) ;; Load 3 seconds after startup
#+END_SRC
* Performance Optimization: Garbage Collection
:PROPERTIES:
:LAST_UPDATED: [2025-03-03]
:CHANGELOG: Reduced startup lag by adjusting garbage collection threshold.
:END:

#+NAME: gc-optimization
#+BEGIN_SRC emacs-lisp :tangle yes
(setq gc-cons-threshold (* 50 1000 1000)) ;; Increase garbage collection threshold
(add-hook 'emacs-startup-hook
          (lambda () (setq gc-cons-threshold (* 2 1000 1000)))) ;; Reset after startup
#+END_SRC
